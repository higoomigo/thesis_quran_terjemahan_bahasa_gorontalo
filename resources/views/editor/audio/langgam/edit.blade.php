    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="bg-gray-50 px-6 py-4 border-b border-gray-100">
                <h3 class="text-lg font-bold text-gray-800">Edit Langgam: {{ $langgam->nama }}</h3>
            </div>
            
            <form action="{{ route('editor.audio.langgam.update', $langgam->id) }}" method="POST" class="p-6 space-y-5">
                @csrf
                @method('PUT')
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-1">Nama Langgam</label>
                    <input type="text" name="nama" value="{{ old('nama', $langgam->nama) }}" required class="w-full rounded-lg border-gray-300 bg-gray-50 py-2.5 px-3 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                        @php
                            // Ambil semua ID Qari yang sudah terhubung dengan langgam ini
                            $selectedQaris = $langgam->pelantuns->pluck('id')->toArray();
                        @endphp
                        
                        @foreach($qaris as $qari)
                        <label class="cursor-pointer">
                            <input type="checkbox" name="qari_ids[]" value="{{ $qari->id }}" class="peer hidden" 
                                {{ in_array($qari->id, $selectedQaris) ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="3" class="w-full rounded-lg border-gray-300 bg-gray-50 py-2 px-3 text-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('deskripsi', $langgam->deskripsi) }}</textarea>
                </div>
